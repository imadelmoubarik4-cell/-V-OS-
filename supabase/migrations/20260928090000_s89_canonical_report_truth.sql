-- S89 canonical report truth.
--
-- One purchasing-spend definition across Reports, Atlas AI and the browser
-- (apps/web/assets/js/atlas-stock-truth.js purchaseSpend, _shared/atlas-domain
-- purchaseSpend): spend is costed purchase receipts in the period. A receipt
-- is a 'restock' movement (the receiving path, public.adjust_inventory; the
-- names purchase/delivery/receive/receipt are accepted for imported history)
-- with a positive quantity_change. Its amount is total_cost when positive,
-- else unit_cost x quantity_change when unit_cost is positive; otherwise the
-- receipt is uncosted (counted, not valued). Waste, sales, counts, transfers
-- and adjustments are never spend; waste stays its own section.
--
-- S86.1 summed positive total_cost over every movement type, so costed waste
-- was counted as purchasing spend. This migration replaces
-- atlas_private.reports_snapshot_v2 with the S86.1 body plus:
--   * purchasing spend, comparison spend, supplier spend and price changes
--     read purchase receipts only (the canonical rule above);
--   * the purchasing summary reports uncosted receipts;
--   * the reporting period uses the venue clock (atlas_private.venue_timezone
--     and venue_business_date) instead of a hard-coded time zone.
-- Signature, owner, SECURITY INVOKER, STABLE, search_path '' and grants are
-- unchanged (CREATE OR REPLACE keeps the ACL). No table, row or policy changes.
-- Acceptance: scripts/verify_s89_report_truth_preview.sql.
...
declare
  ...
  -- S89: the venue clock (Settings -> venue.timezone, business date), not a
  -- hard-coded zone.
  venue_zone text := atlas_private.venue_timezone();
  period_start date := coalesce(p_period_start,atlas_private.venue_business_date());
  period_end date := coalesce(p_period_end,atlas_private.venue_business_date());
  purchase_uncosted integer := 0;
...
begin
  ...
  start_inclusive := period_start::timestamp at time zone venue_zone;
  end_exclusive := (period_end+1)::timestamp at time zone venue_zone;
  comparison_start_inclusive := case when p_comparison_start is null then null else p_comparison_start::timestamp at time zone venue_zone end;
  comparison_end_exclusive := case when p_comparison_end is null then null else (p_comparison_end+1)::timestamp at time zone venue_zone end;
  ...
  receipt_rows as (
    -- S89 canonical purchase receipt (atlas-domain purchaseReceiptAmount):
    -- a restock/receipt movement with a positive quantity. Waste, sales,
    -- counts, transfers and adjustments are never spend.
    select movement.*,case when coalesce(movement.total_cost,0)>0 then movement.total_cost when coalesce(movement.unit_cost,0)>0 then movement.unit_cost*movement.quantity_change else null end as receipt_amount
    from movement_input movement
    where lower(coalesce(movement.movement_type,'')) in ('restock','purchase','delivery','receive','receipt') and coalesce(movement.quantity_change,0)>0
  ),
  period_rows as (
    select movement.*,supplier.name as supplier_name
    from receipt_rows movement
  ...
  select count(*) filter (where receipt_amount is not null)::integer,
    count(*) filter (where receipt_amount is null)::integer,
    coalesce(sum(receipt_amount),0),
    coalesce(jsonb_agg(jsonb_build_object(
      'id',id,'item_id',item_id,'item_name',item_name,'movement_type',movement_type,
      'quantity_change',quantity_change,'unit_cost',unit_cost,'total_cost',total_cost,'amount',receipt_amount,
      'supplier',coalesce(supplier_name,'Unassigned'),'created_at',created_at,'note',note
    ) order by created_at desc),'[]'::jsonb)
  into purchase_movements,purchase_uncosted,purchase_spend,purchase_rows
  from period_rows;
  ...
  if comparison_start_inclusive is not null and comparison_end_exclusive is not null then
    ...
    select count(*) filter (where (case when coalesce(total_cost,0)>0 then total_cost when coalesce(unit_cost,0)>0 then unit_cost*quantity_change else null end) is not null)::integer,
      coalesce(sum(case when coalesce(total_cost,0)>0 then total_cost when coalesce(unit_cost,0)>0 then unit_cost*quantity_change else null end),0)
    ...
    where created_at>=comparison_start_inclusive and created_at<comparison_end_exclusive
      and lower(coalesce(movement_type,'')) in ('restock','purchase','delivery','receive','receipt') and coalesce(quantity_change,0)>0;
  end if;
  ...
  grouped as (
    select coalesce(name,'Unassigned') as supplier_name,
      count(movement.id) filter (where movement.created_at>=start_inclusive and movement.created_at<end_exclusive and lower(coalesce(movement.movement_type,'')) in ('restock','purchase','delivery','receive','receipt') and coalesce(movement.quantity_change,0)>0)::integer as movement_count,
      coalesce(sum(case when coalesce(movement.total_cost,0)>0 then movement.total_cost when coalesce(movement.unit_cost,0)>0 then movement.unit_cost*movement.quantity_change else null end) filter (where movement.created_at>=start_inclusive and movement.created_at<end_exclusive and lower(coalesce(movement.movement_type,'')) in ('restock','purchase','delivery','receive','receipt') and coalesce(movement.quantity_change,0)>0),0) as spend,
  ...
  reports := jsonb_build_object(
    ...
    'purchasing',jsonb_build_object('summary',jsonb_build_object('spend',purchase_spend,'movement_count',purchase_movements,'uncosted_receipts',purchase_uncosted,'open_purchase_orders',null,'overdue_orders',null,'receiving_differences',null),'comparison',jsonb_build_object('spend',compare_purchase_spend,'movement_count',compare_purchase_movements),'formula','Purchasing spend is costed purchase receipts in the period: restock (receipt) movements with a positive quantity, valued at total_cost, else unit_cost times quantity. Receipts without a cost are counted, not valued. Waste and adjustments are reported separately, never as spend.','rows',purchase_rows,'price_changes',price_rows),
  ...
  return jsonb_build_object(
    'version','atlas-reports/0.2.1','generated_at',generated_at_value,'timezone',venue_zone,'currency','ISK',
    ...
    'trust',jsonb_build_object('reports_are_read_only',true,'sales_values_invented',false,'source_data_modified',false,'permission_sensitive_data_loaded_for_staff',false,'reykjavik_reporting_timezone',venue_zone='Atlantic/Reykjavik','venue_reporting_timezone',venue_zone,'purchasing_spend_is_costed_receipts',true,'currency','ISK')
  );
end;
$function$;
