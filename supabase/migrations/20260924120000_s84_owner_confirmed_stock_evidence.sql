...
begin
  ...
  where lower(coalesce(source_type, '')) in ('owner_confirmed', 'owner_confirmed_supplier_price')
...
declare
  ...
  owner_types constant text[] := array['owner_confirmed', 'owner_confirmed_supplier_price'];
...
as $$
  select
    ...
    item.updated_at,
    case
      when lower(coalesce(item.source_type, '')) in ('owner_confirmed', 'owner_confirmed_supplier_price')
