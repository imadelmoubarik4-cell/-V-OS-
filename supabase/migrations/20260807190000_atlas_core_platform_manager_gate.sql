-- P2.1 manager gate repair.
--
-- Core-platform release readiness contains infrastructure health, operational
-- error summaries and release-gate evidence. It is therefore a manager/admin
-- surface even though the RPC itself remains service-role-only. The original
-- function called the shared role helper with p_manager_required=false, which
-- allowed a server caller to request this snapshot using a bartender role.
...
comment on function atlas_private.core_platform_snapshot(uuid,text) is
  'Manager/admin-only P2.1 readiness snapshot behind the service-role RPC boundary.';
