...
async function productionInventory(context: Context): Promise<JsonObject[]> {
  ...
  url.searchParams.set(
    "select",
    "id,name,category,quantity,unit,par_level,bin_location,sku,barcode,updated_at,source_updated_at,active",
  );
  ...
  return parsed.filter((row): row is JsonObject => Boolean(row) && typeof row === "object" && !Array.isArray(row));
}
...
async function detail(context: Context, sessionId: string) {
  ...
  return { count, staff: actorPayload(context) };
}
    ...
    switch (action) {
      ...
      case "save-line": {
        ...
        const countMethod = status === "counted"
          ...
          : null;
        result = await branchRpc("atlas_stock_count_save_line", {
          p_session_id: sessionId,
          p_line_id: requiredUuid(body.line_id, "Count line"),
          p_line_status: status,
          p_observed_quantity: status === "counted" ? quantityValue(body.observed_quantity, "Observed quantity") : null,
