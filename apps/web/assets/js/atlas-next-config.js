...
(() => {
  ...
  function recipeModalMarkup() {
    return `<section class="atlas-modal-panel" data-modal-panel role="dialog" aria-modal="true" aria-labelledby="recipe-modal-title">
      <div class="atlas-modal-header"><div><h2 id="recipe-modal-title">New recipe</h2><p id="recipe-modal-subtitle">Build a service-ready recipe connected to live inventory.</p></div><button type="button" class="atlas-modal-close" data-modal-close aria-label="Close recipe editor">×</button></div>
      <form id="recipe-form" style="display:flex;flex-direction:column;min-height:0;"><input type="hidden" id="recipe-id" /><div class="atlas-modal-body">
        <section class="atlas-form-section"><div class="atlas-section-heading"><div><h3>Recipe details</h3><p>The information your team needs to identify and serve the recipe.</p></div></div><div class="atlas-form-grid"><div class="atlas-field"><label for="recipe-name">Recipe name *</label><input type="text" id="recipe-name" required placeholder="e.g. Espresso Martini" /></div><div class="atlas-field"><label for="recipe-category-id">Category *</label><select id="recipe-category-id" required></select></div><div class="atlas-field"><label for="recipe-glassware">Glassware</label><input type="text" id="recipe-glassware" placeholder="e.g. Coupe" /></div><div class="atlas-field"><label for="recipe-garnish">Garnish</label><input type="text" id="recipe-garnish" placeholder="e.g. 3 coffee beans" /></div><div class="atlas-field full"><label for="recipe-image-url">Image URL</label><input type="url" id="recipe-image-url" placeholder="https://…" /></div></div></section>
        <section class="atlas-form-section"><div class="atlas-section-heading"><div><h3>Inventory-linked ingredients</h3><p>Select real inventory items. Atlas keeps the recipe connected when stock data changes.</p></div></div><div class="ingredient-builder-v2"><div class="ingredient-column"><label for="ingredient-item">Inventory item</label><select id="ingredient-item"></select></div><div class="ingredient-column"><label for="ingredient-qty">Quantity</label><input type="number" id="ingredient-qty" min="0" step="0.01" placeholder="45" /></div><div class="ingredient-column"><label for="ingredient-unit">Unit</label><select id="ingredient-unit"><option value="ml">ml</option><option value="g">g</option><option value="each">each</option><option value="bottle">bottle</option><option value="can">can</option><option value="dash">dash</option><option value="tsp">tsp</option></select></div><button type="button" class="ingredient-add-button" id="add-ingredient-btn">Add ingredient</button></div><div id="ingredient-list" class="ingredient-list-v2"></div></section>
        <section class="atlas-form-section"><div class="atlas-section-heading"><div><h3>Service specification</h3><p>Method, yield and the details that create consistency during service.</p></div></div><div class="atlas-form-grid"><div class="atlas-field full"><label for="recipe-method">Method</label><textarea id="recipe-method" placeholder="Shake hard with ice, then fine strain into a chilled coupe."></textarea></div><div class="atlas-field"><label for="recipe-yield-qty">Recipe yield</label><input type="number" id="recipe-yield-qty" min="0.01" step="0.01" value="1" /></div><div class="atlas-field"><label for="recipe-yield-unit">Yield unit</label><input type="text" id="recipe-yield-unit" value="serving" /></div><div class="atlas-field full"><label for="recipe-notes">Service notes</label><textarea id="recipe-notes" placeholder="Allergens, substitutions, prep notes or service guidance."></textarea></div></div></section>
        <section class="atlas-form-section"><div class="atlas-section-heading"><div><h3>Pricing and availability</h3><p>Costing uses the linked inventory cost and package size where available.</p></div></div><div class="atlas-form-grid"><div class="atlas-field"><label for="recipe-menu-price">Menu price (ISK)</label><input type="number" id="recipe-menu-price" min="0" step="1" placeholder="3290" /></div><div class="atlas-field"><label>Availability</label><div class="recipe-toggle-row"><label class="recipe-check"><input type="checkbox" id="recipe-active" checked />Active for service</label><label class="recipe-check"><input type="checkbox" id="recipe-show-on-menu" checked />Show on public menu</label></div></div></div><div class="recipe-cost-strip"><div class="recipe-cost-box"><span>Total recipe cost</span><strong id="calc-total-cost">0 ISK</strong></div><div class="recipe-cost-box"><span>Cost per serving</span><strong id="calc-cost-per-serving">0 ISK</strong></div><div class="recipe-cost-box"><span>Cost percentage</span><strong id="calc-cost-pct">—</strong></div><div class="recipe-cost-box"><span>Profit per serving</span><strong id="calc-profit">—</strong></div></div><p class="recipe-cost-note" id="recipe-cost-note">Add linked ingredients to calculate the recipe cost.</p><div class="recipe-availability-note" id="recipe-availability-note"><i data-lucide="package-check"></i><span>Add linked ingredients to calculate current service availability.</span></div></section>
      </div><div class="atlas-modal-footer"><span class="recipe-save-state" id="recipe-save-state"></span><div class="atlas-modal-actions"><button type="button" class="atlas-modal-secondary" data-modal-close>Cancel</button><button type="submit" class="atlas-modal-primary">Save recipe</button></div></div></form>
    </section>`;
  }
  ...
  function normalizeImportCenter() {
    ...
    if (pipeline) pipeline.innerHTML = '<span data-stage="uploaded">Uploaded</span><span data-stage="reading">Reading</span><span data-stage="extracting">Extracting</span><span data-stage="matching">Matching</span><span data-stage="ready">Review</span><span data-stage="importing">Importing</span>';
  }
