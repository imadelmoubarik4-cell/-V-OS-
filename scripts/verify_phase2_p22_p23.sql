...
create temporary table phase2_acceptance_results(
  test_name text primary key,
  passed boolean not null,
  detail text not null
) on commit drop;

do $phase2_acceptance$
declare
...
begin
  insert into atlas_private.import_batches(
    ...
    '11000000-0000-4000-8000-000000000001','p22-contract-source','knowledge',
  ...
  insert into phase2_acceptance_results values (
    ...
    'Snapshot returned metadata and freshness without source bodies, storage paths or credentials.'
  );
  ...
  insert into phase2_acceptance_results values (
    ...
    'Bartender cannot query the private source metadata gateway.'
  );

  insert into phase2_acceptance_results
  select 'p22_private_rls',
         ...
         'Knowledge source attribution and read-source event evidence remain private.'
  ...
  insert into phase2_acceptance_results values (
    ...
    'P2.2 public wrapper is service-role-only.'
  );

  select atlas_private.pos_candidate_score('VÁ Espresso-Martini','VA Espresso Martini')
  into exact_score;
  insert into phase2_acceptance_results values (
    'checkpoint_m_normalization',exact_score=1,
    'Icelandic accents and punctuation normalize to an exact deterministic match.'
  );

  check_payload := public.atlas_connections_begin_check(
    'dineout',gen_random_uuid(),'contract','acceptance',actor_id,'Phase 2 acceptance','admin'
  );
  check_id := (check_payload->>'id')::uuid;
  perform public.atlas_connections_finish_check(
    check_id,'healthy','passed',1,null,'Checkpoint M contract fixture connection.',
  ...
  perform public.atlas_pos_mapping_refresh_targets(
    jsonb_build_array(
      jsonb_build_object(
        ...
        'name','Espresso Martini','product_type','cocktail','menu_price',2500,
        'show_on_menu',true,'active',true
      ),
      jsonb_build_object(
        'production_recipe_id','12000000-0000-4000-8000-000000000002',
        'name','Aperol Spritz','product_type','cocktail','menu_price',2400,
        'show_on_menu',true,'active',true
      )
    ),
    actor_id,'Phase 2 acceptance','admin'
  ...
  into product_id,target_id
  from atlas_private.pos_products product
  join atlas_private.pos_product_candidates candidate on candidate.product_id=product.id
  where product.external_product_id='pos-espresso'
  order by candidate.candidate_rank limit 1;

  insert into phase2_acceptance_results
  select 'checkpoint_m_candidates_pending',
         count(*)=2
           and bool_and(mapping.status='pending')
           and (select count(*) from atlas_private.pos_product_candidates)>=2,
         'Deterministic candidates were generated, but every product remained pending.'
  from atlas_private.pos_product_mappings mapping;

  perform public.atlas_pos_mapping_decide(
    product_id,target_id,'approve','Verified contract fixture',
    actor_id,'Phase 2 acceptance','manager'
  );
  insert into phase2_acceptance_results
  select 'checkpoint_m_manager_approval',
         mapping.status='approved' and mapping.target_id=target_id
           and mapping.decided_at is not null,
         'Manager approval created an explicit approved mapping.'
  from atlas_private.pos_product_mappings mapping
  where mapping.product_id=product_id;

  denied := false;
  begin
    perform public.atlas_pos_mapping_decide(
      product_id,target_id,'approve','Not allowed',
  ...
  insert into phase2_acceptance_results values (
    ...
    'Bartender cannot approve or change POS product mappings.'
  );
  ...
  insert into phase2_acceptance_results values (
    ...
    'Mappings do not enable sales ingestion, Brain sales evidence or ordering.'
  );
  ...
  insert into phase2_acceptance_results values (
    ...
    'Viewer cannot query the manager-only Checkpoint M workspace.'
  );
  ...
  select 'checkpoint_m_private_rls',
         count(*)=7 and bool_and(c.relrowsecurity)
           and bool_and(not has_table_privilege('anon',format('atlas_private.%I',c.relname),'select'))
           and bool_and(not has_table_privilege('authenticated',format('atlas_private.%I',c.relname),'select')),
         'All seven Checkpoint M tables have RLS and no browser-role read grants.'
  from pg_class c join pg_namespace n on n.oid=c.relnamespace
  where n.nspname='atlas_private'
    and c.relname in (
  ...
  insert into phase2_acceptance_results values (
    ...
    'Checkpoint M wrappers are service-role-only.'
  );

  select id into event_id from atlas_private.pos_mapping_events order by created_at limit 1;
  denied := false;
  begin
    update atlas_private.pos_mapping_events set payload=payload||'{"tampered":true}'::jsonb
    where id=event_id;
  exception when others then
    denied := true;
  end;
  insert into phase2_acceptance_results values (
    ...
    'POS mapping event history rejects updates.'
  );
end
$phase2_acceptance$;
...
from phase2_acceptance_results;
