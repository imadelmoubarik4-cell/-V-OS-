-- S92 preview-only acceptance: Messages sender identity
-- (20260930095000_s92_messages_sender_identity.sql). Rolled back.
--
...
--   psql -v ON_ERROR_STOP=1 -X -qAt -f scripts/verify_s92_messages_sender_identity_preview.sql

begin;

create temporary table s92_mi (test_name text primary key, passed boolean not null) on commit drop;
grant all on table s92_mi to public;
...
from (values
  ('00000000-0000-4000-8000-0000000a9201','s92-owner@example.invalid'),
  ('00000000-0000-4000-8000-0000000a9202','s92-bar@example.invalid'),
  ('00000000-0000-4000-8000-0000000a9203','s92-old@example.invalid')) as u(id, email);
insert into public.profiles (id,email,display_name,role,active) values
  ('00000000-0000-4000-8000-0000000a9201','s92-owner@example.invalid',null,'admin',true),
  ('00000000-0000-4000-8000-0000000a9202','s92-bar@example.invalid',null,'bartender',true),
  ('00000000-0000-4000-8000-0000000a9203','s92-old@example.invalid',null,'bartender',true)
on conflict (id) do update set role=excluded.role, active=excluded.active, display_name=excluded.display_name;

-- ---------- the S87 name rule ----------
insert into s92_mi select 'safe_staff_name trims, collapses whitespace and refuses addresses and blanks',
...
insert into s92_mi select 'before the backfill the old Team name is not on the profile',
...
begin
  ...
  insert into s92_mi values ('the backfill copies the Team name once and is a no-op when re-run', first_rows >= 1 and second_rows = 0);
end
$backfill$;
insert into s92_mi select 'the backfilled profile shows its Team name',
...
reset role;
insert into s92_mi select 'saving a Team name writes profiles.display_name (own and manager saves)',
  bool_and(case id
...
reset role;
insert into s92_mi select 'a rename follows', display_name = 'Sara J.' from public.profiles where id = '00000000-0000-4000-8000-0000000a9202';
...
reset role;
insert into s92_mi select 'an email-shaped Team name never becomes the display name',
...
select public.atlas_team_profile_upsert_details('00000000-0000-4000-8000-0000000a9202',null,null,null,null,null,null,'managers_only',null,null,
  '00000000-0000-4000-8000-0000000a9202','Sara J.','bartender');
reset role;
insert into s92_mi select 'clearing the Team name keeps the last display name',
  display_name = 'Sara J.' from public.profiles where id = '00000000-0000-4000-8000-0000000a9202';
...
begin
  ...
  insert into s92_mi values ('the conversation preview carries sender_id and sender_role',
  ...
  insert into s92_mi values ('thread rows keep sender_id, sender_label, is_own and read receipts',
  ...
  insert into s92_mi values ('unread counts are unchanged: the admin has one unread message in General',
    (select (value->>'unread_count')::int from jsonb_array_elements(snap->'channels') where value->>'key' = 'general') >= 1);
end
$snapshot$;

-- ---------- privileges ----------
insert into s92_mi select 'anon and authenticated cannot execute the new or replaced functions',
...
insert into s92_mi select 'the trigger function is a definer with search_path pinned',
...
insert into s92_mi select 'the snapshot stays a security invoker',
